...
type TabType = 'users' | 'roles' | 'audit' | 'health' | 'settings';
...
export default function Admin() {
    ...
    // Settings query
    const { data: settingsData, isLoading: settingsLoading } = useQuery('admin-settings', () =>
        adminAPI.getSettings().then((res) => res.data)
    );
    const [sessionTimeout, setSessionTimeout] = useState(30);

    // Sync sessionTimeout with fetched data when it loads
    useEffect(() => {
        if (settingsData?.session_timeout_minutes) {
            setSessionTimeout(settingsData.session_timeout_minutes);
        }
    }, [settingsData]);

    // Sync sessionTimeout with fetched data
    const updateSettingsMutation = useMutation(
        (data: { session_timeout_minutes: number }) => adminAPI.updateSettings(data),
        {
            onSuccess: () => {
                        ...
                        <div className="space-y-6">
                                ...
                                <div className="max-w-md">
                                    ...
                                    <div className="flex items-center gap-4">
                                        ...
                                        <span className="text-sm text-gray-500">
                                            ({Math.floor(sessionTimeout / 60)}h {sessionTimeout % 60}m)
                                        </span>
                                    </div>
                            ...
                            <div className="card p-6 bg-blue-50 border-blue-200">
                                <div className="flex gap-3">
                                    <Icons.Clock />
                                    <div>
                                        <h4 className="font-medium text-blue-900">About Session Timeout</h4>
                                        <p className="text-sm text-blue-700 mt-1">
                                            Users will be automatically logged out after the specified period of inactivity.
                                            Activity includes clicking, typing, or navigating within the application.
                                        </p>
                                    </div>
                                </div>
                            </div>
